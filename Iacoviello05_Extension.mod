...
//% We replicate theto IRF to a monetary policy shock
...
Varexo ejhat, euhat, eAhat, eRhat;
...
qh1toY = j*s1/(1-beta)+j*m*qhtoY+j*s2/(1-beta2-m2*(beta-beta2-j*(1-beta)));
qh2toY = j*(((1-(1/beta))*psi_i*ytoY)+s2) /(1-beta2-m2*(beta-beta2)-j*m2*(beta-1));
...
b2toY = ((j*beta*m2)*(((1-(1/beta))*psi_i*ytoY)+s2)/(1-beta2-m2*(beta-beta2)-j*m2*(beta-1))) + psi_i*ytoY ;

c2toY = (((1-(1/beta))*psi_i*ytoY)+s2)*(1-beta2-m2*(beta-beta2))/(1-beta2-m2*(beta-beta2)-j*m2*(beta-1));
...
b2hat = (b2toY - psi_i*ytoY)*(qhat(+1)+h2hat-rrhat) + psi_i*ytoY*yhat;
...
b2toY*b2hat = c2toY*c2hat+qh2toY*(h2hat-h2hat(-1))+R*b2toY*(b2hat(-1)+Rhat(-1)-pihat)-(ytoY*yhat);
...
shocks;
var eRhat; stderr sigmaR;
var euhat; stderr sigmau;
var ejhat; stderr sigmaj;
var eAhat; stderr sigmaA;
end;

stoch_simul(order=1, irf=20, nograph, noprint) Rhat pihat qhat Yhat chat c1hat c2hat;
